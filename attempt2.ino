...
void buzzer_logic(const int s_distance){
  boi[flag] = s_distance1;
  //delay(1000);
  if(flag % 2 == 0){
...
int PrioritySonar(const int d1, const int d2, const int d3, const int d4){
  long shortest_distance = min(d1,min(d2, min(d3,d4)));
  if(shortest_distance == d1)
    return 0;
  else if(shortest_distance == d2)
    return 1;
  else if(shortest_distance == d3)
    return 2;
  else if(shortest_distance == d4)
    return 3;
  else
    return;
}
